...
public class OfficeVisit extends DomainObject<OfficeVisit> {
	 ...
	 *
	 * @param ovf
	 *            OfficeVisitForm The OfficeVisitForm to create an OfficeVisit out
	 *            of
	...
	public OfficeVisit(final OfficeVisitForm ovf) throws ParseException, NumberFormatException {
		...
		try {
			...
			at = AppointmentType.GENERAL_CHECKUP; /*
													 * If for some reason we don't have a type, default to general
													 * checkup
													 */
		}
		setType(at);
		...
		if (ps != null) {
			setPrescriptions(
					ps.stream().map((final PrescriptionForm pf) -> new Prescription(pf)).collect(Collectors.toList()));
		}
	...
	/**
	 * Validates an office visit form for containing correct fields for patients 12
	 * and over. Expects the basic health metrics to already be set by the
	...
	/**
	 * Validates an office visit form for containing correct fields for patients 3
	 * and under.
	 */
	 ...
	 * serialized or saved in DB If removed, serializer gets into an infinite loop
	 */
	...
	/**
	 * Overrides the basic domain object save in order to save basic health metrics
	...
	public void save() {
		...
		this.getLabProcedures().forEach(lp -> {
			...
			if (lpIsSaved) {
				LoggerUtil.log(TransactionType.EIDT_LAB_PROCEDURE, LoggerUtil.currentUser(), getPatient().getUsername(),
						"Editing lab procedure with id " + lp.getId());
			} else {
	...
	public void delete() {
		if (diagnoses != null) {
			for (final Diagnosis d : diagnoses) {
				d.delete();
				try {
					LoggerUtil.log(TransactionType.DIAGNOSIS_DELETE, getHcp().getUsername(), getPatient().getUsername(),
							getHcp() + " deleted a diagnosis for " + getPatient());
				} catch (final Exception e) {
					e.printStackTrace();
				}
			}
	...
	/**
	 * Deletes all Office visits, and all Diagnoses (No diagnoses without an office
	 * visit) and all Lab Procedure (No lab procedure without an office visit)
	 */
